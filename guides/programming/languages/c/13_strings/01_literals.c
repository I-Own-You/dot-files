...
// 2.1 be cautious wiht octal and hexa sequences indie strings, octal
...
// 4.2 "abc", its stores as an array with "a", "b", "c", \0 elements, string elements can be,
//     empty "", empty elements are stored as a single null character \0, and the whole string,
//     literal is of type char*, so char *my_str = "abc";
//     
...
// REMEMBER, char *p = "a"; IS A STRING LITERAL, NOT ARRAY.

// 5. where is the string literal stored ?
//    its memory address resembles in a special are inside, memory layout which is READ-ONLY,
//    
