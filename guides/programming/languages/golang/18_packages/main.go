...
import (
	...
	//			   "math" itself is a package too, and it contains another package "rand"
)
...
// there is also a function that will run after all variables are initialized (outside of functions)
// in all packages, and after all packages are imported, before the main() execution. (global variables)
//
...
//      		2. init functions from current package in case there are multiple files of same
//				   pkg you cant guarantee the order. (order is determined by go toolchain)
